...
- `PREDECESSOR: NIGHT-AAA-029 PENDING / STOP_EVIDENCE_INSUFFICIENT — processed by JOBS CYCLE 030.`

### PRIMARY

1. Preflight GitHub vivo + duplicate-check. Reutiliza SAME #69; no branch/PR replacement.
2. Acepta la evidencia existente: helper `webBulkSave.ts` + focused tests están probados y el exact head `b2ab75ae...` tuvo Desktop Portability `33303237401`, D6 `33303237410` y D7 `33303237375` SUCCESS. No rerun ceremonial sobre head sin cambios.
3. El gap factual es product wiring: `src/App.tsx` mantiene `handleReviewedSaveAll` como ruta separada y no consume `saveAllWebItems`/su summary explícito. Implementa únicamente el wiring mínimo App/Review/Import/Bulk necesario para reutilizar el coordinator existente y preservar la durable/CAS boundary por item.
4. No reimplementes coordinator, durable single-item commit, uploads ni server garbage journal/#70.
5. Añade focused wiring evidence que demuestre al menos: ruta productiva usa coordinator; saved/conflict/failed se conservan sin colapsar fallos; partial failure continúa; retry no reenvía items ya durablemente guardados.
...
- Existing exact-head CI: Desktop Portability `33303237401` SUCCESS; D6 `33303237410` SUCCESS; D7 `33303237375` SUCCESS; Upgrade skipped/no aplicable.
- Helper/unit evidence is green, but product audit found `App.tsx -> handleReviewedSaveAll` still bypasses `saveAllWebItems` and collapses failures into the older path.
- No product wiring evidence, no merge. Issue #41 handoff `5468039685`.

## HISTORIAL COMPACTO

- `NIGHT-AAA-030`: ASSIGNED — SAME #69 minimal product wiring + fresh CI + merge if green; fallback NONE.
