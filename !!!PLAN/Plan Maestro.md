...
- **Último avance:** M0-C de Tarea 5.1 quedó **probado live en su alcance acotado** en el draft PR BeatGaler #17 `test(security): prove M0-C temp-auth renewal`, rama `task-5.1-temp-auth-renewal-probe`, head `64308f3297304907972e777200db04b38119c7c0`, apilado sobre B2. El diff sigue siendo solo de probe (`scripts/telegram-temp-auth-renewal-live.mjs` + `.github/workflows/probe-task-5.1-temp-auth-renewal.yml`) y no toca runtime. El workflow dedicado `32791160563`, job `97632841439`, terminó PASS en production DC 1: se crearon/bindearon tres temp keys A/B/C; A hizo RPC directo, B se generó y bindó mientras A seguía primaria, luego B fue promovida y A quedó retenida en el slot secondary; la misma operación lógica continuó bajo B; después un fault injection local borró el estado temp antes de cualquier RPC adicional y C se generó/bindó para continuar con la misma identidad bot. Evidencia: `proactive_renewal_proven=true`, `old_key_secondary_slot_retention_proven=true`, `logical_operation_continuity_proven=true`, `early_expiration_recovery_simulated=true`, `server_early_expiration_proven=false`, `wall_clock_expiry_waited=false`, `permanent_auth_reaches_client=false`, `bot_token_reaches_client=false`, `api_hash_reaches_client=false`, `galer_file_bytes=false`, `vault_used=false` y `token_rotation_or_revoke=false`. El head inicial `f4b030aad599526b8ade15c07ba6b376e2603c52` falló dos veces antes del primer bind porque el sentinel random de compatibilidad mtcute se armaba antes del fresh temp handshake; el head final restauró el orden ya probado en B2 — generar temp primero y armar sentinel después — y pasó sin tocar producto. CI normal #132 (`32791160610`) terminó **PASS 6/6** en el mismo head: Web/shared, Supply chain, Windows, macOS arm64, macOS x86_64 y `Required CI` success.
- **Próximo paso principal:** M0-D aislado: demostrar data plane directo hasta el límite de producto de **1.9 GB** sin relay de Galer Cloud. Preferir una transferencia real suficientemente larga que atraviese una renovación de temp key para convertir la continuidad acelerada de M0-C en evidencia de operación larga real; si M0-D no cruza renovación, ese subgate seguirá abierto aparte.
...
- **Bloqueos actuales:** historial Git antiguo de información operacional aún pendiente de decisión de purga; dominio, Apple Developer, Authenticode y revisiones externas de 1.2 pendientes; regresión de tiempo de carga inicial de librería registrada para 12.1; 5.1 no puede aprobarse hasta demostrar 1.9 GB y operación larga real, cubrir expiración server-side/natural si se mantiene como gate, aislamiento/escalabilidad, migración productiva sin credencial compartida, hardening restante y revisión independiente. M0-C prueba renovación proactiva y recovery ante pérdida/expiración **simulada**, no una expiración temprana inducida por Telegram ni una espera a expiry natural.
