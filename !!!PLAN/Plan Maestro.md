...
- **Último avance:** M0-E1 de Tarea 5.1 quedó **probado live para los tres targets Desktop de CI** en el draft PR BeatGaler #23 `test(security): prove temp auth across Windows and macOS`, rama `task-5.1-temp-auth-cross-platform-probe`, head `8ea250cf7b71dda6d09fd408deb473a9151e7d2e`, apilado sobre M0-D. En el workflow #173 (`32827228117`) los jobs `M0-E1 temp auth live` de **Windows x86_64, macOS arm64 y macOS x86_64** terminaron PASS. Los tres ejecutaron el mismo B2 real contra production DC 1 y probaron `bot_identity_proven=true`, `network_bind_proven=true`, `direct_mtproto_operation_proven=true`, con `permanent_auth_reaches_client=false`, `bot_token_reaches_client=false`, `api_hash_reaches_client=false`, `galer_file_bytes=false`, `vault_used=false` y `token_rotation_or_revoke=false`. El probe no toca runtime productivo. El CI normal del mismo run sigue en progreso al registrar este avance, por lo que **no se declara todavía `Required CI` PASS para #23**.
- **Próximo paso principal:** continuar Tarea 5.1 con la mitad todavía pendiente del subgate cross-platform: demostrar la misma frontera temporal en **Web pura dentro de navegador/Worker real**, no en un proceso Node que solo simule browser. Después cubrir delete propio/cross-bot, aislamiento cross-vault/shared-bot, escalabilidad/admission control, expiración server-side/natural si sigue siendo requisito y finalmente migrar el runtime productivo bajo la frontera aprobada.
...
- **Bloqueos actuales:** historial Git antiguo de información operacional aún pendiente de decisión de purga; dominio, Apple Developer, Authenticode y revisiones externas de 1.2 pendientes; regresión de tiempo de carga inicial de librería registrada para 12.1; 5.1 no puede aprobarse hasta cubrir Web pura, expiración server-side/natural si se mantiene como gate, aislamiento/escalabilidad, delete propio/cross-bot, migración productiva sin credencial compartida, hardening restante y revisión independiente. M0-E1 sí demuestra la frontera B2 live en Windows y ambos macOS sobre runners nativos, pero no prueba navegador Web, runtime productivo, vault, archivo, delete/pin ni aislamiento cross-vault.
