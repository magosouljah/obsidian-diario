...
- **Estado general:** 🔴 `NO-GO` para lanzamiento público. Tareas 4.1, 4.2 y 5.1 quedan cerradas con evidencia. En 5.2 el trabajo software-only exigible por el ADR está cubierto y PostgreSQL ya es la autoridad productiva del control-plane. WAVE 3 congeló cuatro criterios de salida: **el criterio 1 — aceptación independiente de rollback/durabilidad — ya está SATISFECHO por AAA**; siguen abiertos **2) restore independiente representativo + RPO/RTO, 3) rotación productiva multiversión y 4) observabilidad/alertas/on-call/rollback authority**. No iniciar tareas posteriores.
- **Último avance técnico confirmado:** AAA terminó la revisión independiente WAVE 3 y aceptó la evidencia de durabilidad tras restart y de rollback readiness/dry-run desde CURRENT PostgreSQL; no solicita repetir restart/cutover/import/migrations. BBB terminó la auditoría read-only de observabilidad + rotación y confirmó que **no hay un gap software que justifique otra rama**: el contrato multiversión está integrado/verde y lo que falta es evidencia operativa/productiva + ownership explícito. **5.2 sigue `[ 🟡 ] / NO-GO`.**
- **Próximo paso principal para WOZ:** ejecutar el **restore independiente representativo** y medir evidencia real de **RPO <=15 min / RTO <=2 h**. Después, usar el checklist BBB como ruta corta: fijar on-call/rotation operator/rollback authority + routing de alarmas, y ejecutar una única rotación productiva N→N+1 aprobada conservando N durante la ventana de rollback. **No repetir cutover, import JSON, migrations ni la prueba de restart de durabilidad ya demostrada.**
- **Trabajo paralelo elegible:** los handoffs iniciales WAVE 3 de AAA/BBB están `DONE`. Para conservar independencia sin duplicar ejecución, `AAA` queda como reviewer del paquete de restore/RPO-RTO cuando WOZ publique evidencia sanitizada; `BBB` queda como reviewer del paquete productivo de rotación + observabilidad cuando WOZ publique esa evidencia. Tareas 1.2 y 2.2 siguen siendo paralelos permitidos, pero no desplazan el cierre de 5.2. **Apple Developer = `PENDING — DEFERRED`** hasta retomar su gate de firma/notarización. Las asignaciones transitorias viven en Issue #41.
- **Bloqueos actuales:** el marco WAVE 3 conserva cuatro criterios. **(1) SATISFECHO:** aceptación independiente de la evidencia actual de rollback/durabilidad por AAA. **Abiertos:** **(2)** restore independiente representativo con RPO <=15 min y RTO <=2 h medidos; **(3)** evidencia productiva de rotación multiversión; **(4)** observabilidad, alertas, on-call y rollback authority asignados/probados. BBB detectó además que antes de la rotación deben nombrarse explícitamente `on-call owner`, `rotation operator` y `rollback authority`, con routing/ACK de alarmas críticas sanitizado. Fuera de 5.2 siguen pendientes la decisión de purga del historial Git de 2.2, dependencias externas de 1.2 —con Apple Developer diferido—, la regresión de tiempo de carga inicial registrada para 12.1 y el gate global de capacidad 2× con pico propuesto concreto. Riesgos residuales aceptados en 5.1: shared-bot fallback cross-vault cuando no haya bots libres y cleanup físico cross-bot >48 h como deuda durable de GC con INDEX como autoridad. Además, existe deuda preexistente de supply chain/licencia: `telegram@2.26.22` → `@cryptography/aes@0.1.1` GPL-3.0-or-later; no se aprobó GPL globalmente y debe resolverse/revisarse antes de release. La revisión independiente externa sigue pendiente como gate global.
...
- **Evidencia 5.2 — EN PROGRESO `[ 🟡 ]`:** #29–#36 integraron ADR/schema/migrations/envelope encryption/importador/garbage journal/PostgreSQL 16/sagas/reconciliation/recovery/runtime authority/cutover controlado/rollback y worker durable. #37 añadió migration `0004`, staging durable separado de READY, sealed snapshot bundles, quarantine fail-closed, rechazo de final delta stale, binding a digest externo y commit READY atómico; merge `edad9e324132fa086ef729ef4faec574661578a9`, post-merge #272 (`33008305423`) PASS completo, probe #67 (`33008305463`) PASS y recovery artifact `sha256:8a7eb2ad2010da256a296ce66aa18f33e4d40153fda278a92da8d48fdc680e53`. #38 se cerró sin merge al quedar supersedido por #37. #39 añadió keyring versionado y `rotateStoredControlPlaneSecrets`; restore aislado demuestra ciphertext-only, wrong/missing-key rejection, v7→v8 con round-trip y rollback atómico. Merge #39 `1a5cc387aef431cd5f5115ad537f55e80856fb08`; CI post-merge #274 PASS; recovery artifact digest `sha256:0053159a8e21be62e62e72a6996b5ca7baf97ee857db7655d4f827ecd99fc93c`. #40 integró la frontera software AWS Secrets Manager; merge `f997415c794c74ee1b86ef593476dba3587eeca1`; #42 endureció base64 canónico; merge `a968122127c584b5557b25e70a21eb64f75b3c0e`; post-merge `33118368302` PASS completo incluido `Required CI`. WAVE 3 confirmó PostgreSQL productivo, restart durable y rollback dry-run desde CURRENT PG. **AAA aceptó independientemente ese rollback readiness/dry-run y durabilidad: criterio 1 SATISFECHO. BBB verificó que observabilidad/rotación no tienen gap software pendiente; faltan ejecución/evidencia productiva. Pendiente antes de `[x]`: (2) restore independiente representativo + RPO/RTO; (3) rotación productiva multiversión; (4) observabilidad/alertas/on-call/rollback authority.** No repetir cutover/import/migrations/restart de durabilidad.
