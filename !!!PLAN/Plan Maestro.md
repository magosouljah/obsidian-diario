...
- **Último avance:** M0-D de Tarea 5.1 quedó **probado live en su alcance acotado** en el draft PR BeatGaler #18 `test(security): prove M0-D direct 1.9GB data plane`, rama `task-5.1-direct-1.9gb-probe`, head `ca4c312784242c6d7bc98bc5c55b458823d2de23`, apilado sobre M0-C. El diff sigue siendo solo de probe (`scripts/telegram-temp-auth-data-plane-live.mjs` + `.github/workflows/probe-task-5.1-direct-1.9gb.yml`) y no toca runtime. El workflow dedicado `32796833343`, job `97649760589`, terminó PASS en production DC 1: smoke de 16 MiB y transferencia full exacta de **1,992,294,400 bytes = 3800 × 512 KiB** mediante `upload.saveBigFilePart`; una sola `file_id` cruzó la renovación A→B, B se generó/bindó mientras A seguía primaria y después fue promovida conservando A en secondary. Evidencia: `direct_mtproto_file_parts_proven=true`, `full_product_limit_proven=true`, `proactive_renewal_during_transfer_proven=true`, `same_file_id_across_renewal=true`, `old_key_secondary_slot_retention_proven=true`, `permanent_auth_reaches_client=false`, `bot_token_reaches_client=false`, `api_hash_reaches_client=false`, `galer_cloud_file_bytes=0`, `vault_used=false`, `message_created=false`, `server_materialized_file_proven=false` y `token_rotation_or_revoke=false`. CI normal #136 (`32796833320`) terminó **PASS 6/6** en el mismo head: Web/shared, Supply chain, Windows, macOS arm64, macOS x86_64 y `Required CI` success.
- **Próximo paso principal:** continuar Tarea 5.1 con el siguiente subgate pendiente: validar la misma frontera de acceso temporal en **Windows, macOS y Web pura** sin introducir credenciales compartidas ni relay de archivos; después cubrir delete propio/cross-bot, aislamiento cross-vault/shared-bot, escalabilidad/admission control, expiración server-side/natural si sigue siendo requisito y finalmente migrar el runtime productivo bajo la frontera aprobada.
...
- **Bloqueos actuales:** historial Git antiguo de información operacional aún pendiente de decisión de purga; dominio, Apple Developer, Authenticode y revisiones externas de 1.2 pendientes; regresión de tiempo de carga inicial de librería registrada para 12.1; 5.1 no puede aprobarse hasta cubrir expiración server-side/natural si se mantiene como gate, aislamiento/escalabilidad, validación Windows/macOS/Web pura, delete propio/cross-bot, migración productiva sin credencial compartida, hardening restante y revisión independiente. M0-D sí prueba transferencia directa del límite de producto y una operación larga real que cruza renovación A→B, pero no prueba materialización final en vault, descarga posterior ni expiración temprana/natural inducida por Telegram.
