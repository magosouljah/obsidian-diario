...
- **Último avance:** M0-B1 de Tarea 5.1 quedó probado en el draft PR BeatGaler #15, rama `task-5.1-temp-auth-live-bind-probe`, head limpio `2b942deea108fc4818bbb1c088db2f144f3c42c0`, apilado sobre PR #14. El probe usa exclusivamente Telegram TEST DC, sin bot token, API hash, usuario BeatGaler, vault ni archivos. Genera una permanent MTProto auth key en un proceso binder separado y una temporary MTProto auth key en el cliente; el cliente entrega solo metadata/IDs, el binder devuelve únicamente el `encrypted_message`, y el cliente envía `auth.bindTempAuthKey` directamente a Telegram usando la temp key. Telegram devolvió `boolTrue`, por lo que `network_bind_proven=true`; la permanent auth no llegó al cliente y la temp-key nunca llegó al binder. El probe dedicado terminó PASS (run #16) y el mismo head tiene CI completo PASS #117 6/6 incluido `Required CI`. Un experimento posterior de RPC post-bind falló y fue retirado del scope probado; por tanto `direct_mtproto_operation_proven=false` sigue vigente.
- **Próximo paso principal:** M0-B2. Probar identidad bot autorizada y una operación MTProto directa usando la temporary auth key mientras la permanent bot auth/credenciales quedan únicamente del lado controlado. B2 debe seguir sin crear/migrar vaults reales ni transferir archivos de usuario; cualquier secreto se maneja server-side/CI y nunca se imprime ni llega al cliente. Solo después se evalúan renovación/expiración anticipada y data plane 1.9 GB.
