...
- **Último avance:** M0-B2 de Tarea 5.1 quedó **probado live** en el draft PR BeatGaler #16 `test(security): prove M0-B2 bot temp-auth identity`, rama `task-5.1-temp-auth-bot-probe`, head `5ff0d70edd6c4ac11ff54bbd52a68246342130ac`, apilado sobre el B1 probado. El diff sigue limitado a `scripts/telegram-temp-auth-bot-live.mjs` y `.github/workflows/probe-task-5.1-temp-auth-bot.yml`; no toca runtime. El binder es el único proceso que recibe API id/hash + bot token, sigue `USER_MIGRATE_N` del lado controlado y autoriza allí la permanent auth key; el cliente se lanza sin esos secretos, genera/conserva temp auth, recibe solo el DC seleccionado + envelope y hace `auth.bindTempAuthKey` y `users.getUsers(inputUserSelf)` directamente con Telegram. El rerun limpio del workflow B2 `32789070730`, job `97626995224`, terminó PASS en production DC 1 con `bot_identity_proven=true`, `network_bind_proven=true`, `direct_mtproto_operation_proven=true`, `permanent_auth_reaches_client=false`, `bot_token_reaches_client=false`, `api_hash_reaches_client=false`, `galer_file_bytes=false`, `vault_used=false` y `token_rotation_or_revoke=false`. Una primera tentativa del mismo head falló **antes del bind** con `Step 4: invalid nonce hash from server`; el rerun sin cambios pasó, por lo que se registra como ruido transitorio del handshake, no como evidencia negativa del split. El head final además acota el cleanup del probe para que mtcute no retenga CI después del PASS. CI normal #130 (`32789070736`) está en curso al registrar este avance: Web/shared, Supply chain, Windows y macOS arm64 ya PASS; macOS x86_64 y el agregador `Required CI` siguen pendientes.
- **Próximo paso principal:** cerrar la revisión de CI #130 del head B2 y, si termina verde, avanzar a M0-C aislado: demostrar renovación proactiva de temporary auth keys, recuperación ante expiración anticipada/fault injection y continuidad de una operación lógica larga atravesando un cambio de temp key, manteniendo permanent bot credentials únicamente del lado controlado. Después vienen data plane directo de hasta 1.9 GB y la migración del runtime productivo para retirar las credenciales compartidas del cliente.
