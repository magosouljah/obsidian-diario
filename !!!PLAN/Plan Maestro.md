...
- **Último avance:** M0-G de escalabilidad/admission control queda probado en el draft PR BeatGaler #26, rama `task-5.1-pool-scalability-admission`, head `f39578ab0afe40448566087edfdbb62a2c80967a`. El RO fija **máximo 4 vaults activos por transport bot**. El allocator conserva exclusividad preferida y reparto justo por carga; el probe sintético demostró 80 bots × 4 = **320 leases concurrentes**, incluida contención con 16 workers y carga final exacta 4/4/.../4. El allocator anterior aceptaba una 5.ª lease; el nuevo admission control la bloquea, usa waitlist acotada y observable y solo permite shared-bot cuando `freeBots=0`. Workflow dedicado final `32891823812` PASS. CI normal #192 (`32891830172`) terminó **PASS 6/6**, incluido `Required CI`, Web/shared, supply chain, Windows, macOS arm64 y macOS x86_64. Bajo la hipótesis de planeación del RO de 3% de usuarios activos simultáneamente, 320 equivale aproximadamente a 10,666 usuarios totales antes de alcanzar esa demanda esperada; esto es una estimación de capacidad, no un SLA ni sustituye el gate global de carga 2×. Esta evidencia no cierra 5.1 ni cambia el `NO-GO`.
- **Próximo paso principal:** decidir explícitamente si expiración server-side/natural sigue siendo gate; después migrar el runtime productivo Web/Desktop al modelo temporary-auth sin credenciales permanentes compartidas y cerrar discovery/hardening restante. **No iniciar Tarea 5.2 todavía.**
