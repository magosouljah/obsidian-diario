...
- **Último avance:** M0-B0 de Tarea 5.1 quedó probado en el draft PR BeatGaler #14, rama `task-5.1-temp-auth-network-probe`, head `9bd8bee1eda87cbeab051a3937ef95f6c4884ec4`, apilado sobre PR #13 para aislar el incremento. El diff de #14 son solo `scripts/telegram-temp-auth-mtcute-seam.mjs` y una línea de `package.json` que lo incorpora a `test:regressions`; no toca runtime. El probe inspecciona la copia realmente instalada de `@mtcute/web`/`@mtcute/core` y confirmó en CI que ambas son `0.31.0`, que mtcute contiene generación de temporary auth key, `auth.bindTempAuthKey`, envío del bind bajo la temp key y rolling refresh, pero que su PFS stock también consume `permanent auth key` bytes en el mismo cliente. Por tanto **activar `usePfs` tal cual NO satisface 5.1**. CI #111 terminó PASS 6/6 incluido `Required CI` en Web/shared, Supply chain, Windows, macOS arm64 y macOS x86_64. El probe conserva explícitamente `network_bind_proven=false` y `direct_mtproto_operation_proven=false`.
- **Próximo paso principal:** M0-B1 aislado. Probar contra Telegram, preferentemente en test DC y sin tocar identidad/vault productivos, la separación real de dos lados: una permanent MTProto auth key existe solo del lado binder/controlado; el dispositivo genera y conserva una temporary auth key; entrega al binder únicamente metadata/IDs necesarios; recibe el `encrypted_message`; y el dispositivo envía `auth.bindTempAuthKey` **directamente a Telegram** usando la temp key. Solo si Telegram acepta el bind se podrá registrar `network_bind_proven=true`. B1 no debe confundirse con prueba de identidad bot ni data plane funcional; esa evidencia viene después.
