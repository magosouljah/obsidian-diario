...
- **Último avance:** Tarea 4.2 completada. PR BeatGaler #10 integró el gate de supply chain en `integration-v0.8.0-alpha.1`: cero critical/high conocidos en los scanners ejecutados; npm/Cargo advisories, licencias, Gitleaks HEAD-only, SBOM CycloneDX npm+Rust, checksums y procedencia/digest de runtimes; Actions de build/release fijadas por SHA. CI PR #100 PASS en Web/shared, supply chain, Windows, macOS arm64, macOS x86_64 y `Required CI`. El merge real `f6d1f998bd63589ec2ddad7ee4d5818e9b85f016` tiene el mismo árbol `9a7000f3f3a1840ebae0310ac3df6b827561f2c5` que el merge sintético probado por CI.
- **Próximo paso principal:** Tarea 5.1 — aprobar límites de confianza antes de implementar trabajo sensible: capability/proxy tenant-scoped y revocable, origen local/remoto autenticado, parser ID3 vendorized y política CSP/headers/CORS/cookie-CSRF/scopes Tauri.
