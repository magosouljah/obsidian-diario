...
ESTE ES EL CONTEXTO LEELO TODO, SIGUELO ESTRICTAMENTE Y NO OLVIDES ANOTAR AVANCES:
...
**Clasificación de afirmaciones del plan:**

- **CURRENT:** estado observado directamente en los SHAs auditados.
- **MEASURED:** resultado obtenido mediante inventario, build, tests o análisis reproducible durante esta auditoría.
- **TARGET:** objetivo propuesto para aprobar antes del lanzamiento; no describe el estado actual.
- **DECISION:** elección arquitectónica o de producto todavía sujeta a aprobación del owner.
- **GATE:** condición que debe demostrarse con evidencia antes de avanzar o publicar.

Las cifras de esta sección son **MEASURED**, no objetivos. Deben conservar junto al release ledger el SHA, comando o procedimiento utilizado, fecha/hora y resultado para poder reproducirse.

- Web: 317 archivos rastreados; 234 archivos de texto leídos completamente y 83 binarios inventariados.
- Cloud: 281 archivos rastreados; 59 superficies/estados canónicos y 48 rutas HTTP inventariadas. La reconciliación de diseño expande las ausencias agrupadas en S59 y añade superficies explícitas hasta S63; el conteo total debe regenerarse desde el inventario actualizado antes de registrarlo como evidencia medida.
...
Leyenda: **I** implementado y alcanzable, **P** parcial, **H** placeholder, **U** componente existente pero huérfano/no alcanzable, **A** ausente, **D** solo desktop. “Acción” identifica el tratamiento de lanzamiento, no elimina funciones actuales.
...
| S58 | SetupModal huérfano | U | U | eliminarlo o reconstruir onboarding; nunca dejar componente/ruta personal huérfana |
...
| S61 | Sesiones y dispositivos | P | P | inventario de sesiones/dispositivos, revoke-one/revoke-all, límites y feedback de seguridad |
| S62 | Gestión de providers OAuth | P | P | conectar/desconectar Google/X, linking, cancelación, blocked popup, retry y estado visible |
| S63 | Quotas y entitlement reached | P | P | estados explícitos para límite de beats, PROJECT ZIP, dispositivos, sesiones y YouTube; mostrar uso, límite y acción posible |
...
El baseline de release agrupa actualmente **12 P0, 11 P1, 7 P2 y 4 P3**. Son grupos de gate, no conteo de todos los defectos. Las auditorías por rama contienen hallazgos solapados y más detalle visual; no deben sumarse entre sí.
