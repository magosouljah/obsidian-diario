...
- **Último avance:** **PR #34 ya está integrado y su post-merge quedó totalmente verde.** PR #29 integró ADR/threat model/rollback; PRs #30–#32 añadieron schema/migrations, envelope encryption, migración legacy idempotente y garbage journal; PR #33 añadió `pg@8.23.0`, bootstrap fail-closed y PostgreSQL 16 real como gate requerido, corrigiendo además el race real de `schema_migrations` mediante advisory lock previo al ledger. PR #34 añadió operaciones Direct durables, reconciliación forward-only desde el pinned INDEX, detección conservadora de orphans, exporter de rollback desde PG actual y un drill de dump cifrado + restore a una segunda instancia PostgreSQL. #34 se integró como `27ccdcbf41bbce0265730754cc2ff32fd32dc140`; CI post-merge #256 (`32946267360`) terminó PASS completo en PostgreSQL live/recovery, Supply Chain, Web/shared, Windows, macOS arm64/x86_64 y `Required CI`. El probe heredado más reciente #55 (`32946271700`) también terminó PASS sobre el mismo SHA. El artefacto `postgres-recovery-evidence` conserva digest `sha256:09622af6bd0d663b328ce28cb2955245db9fd995d93428e9a2fdeeb3a8c07df1`; el restore verifier de CI observó ~40 ms solo en su dataset pequeño. **Esto prueba el mecanismo de restore en CI, no RTO/RPO productivos. 5.2 no se marca `[x]`.**
- **Próximo paso principal:** implementar el cutover software controlado/default-off con autoridad explícita `json|postgres`, repositories PostgreSQL para el control-plane y rollback fail-closed, sin activar producción ni dual-write indefinido. Después completar worker durable de garbage/reconciliación y adversarial tests restantes. Solo con proveedor PostgreSQL/KMS real, WAL/PITR/retención configurados, RPO <=15 min/RTO <=2 h demostrados y cutover/rollback real con evidencia podrá cerrarse 5.2. No adelantar tareas posteriores.
...
- **BeatGaler integración:** rama `integration-v0.8.0-alpha.1`; HEAD integrado `27ccdcbf41bbce0265730754cc2ff32fd32dc140`; versión `0.8.0-alpha.1`.
...
- **Evidencia 5.2 — EN PROGRESO `[ 🟡 ]`:** arquitectura #29 integrada; #30 añadió schema PostgreSQL/control-plane + constraints + migration contract; #31 añadió migration runner, configuración PostgreSQL fail-closed y secret envelope AES-256-GCM; #32 añadió importador legacy idempotente, garbage journal repository y auth schema compatibility; #33 integró `pg@8.23.0`, bootstrap opt-in fail-closed y PostgreSQL 16 real dentro de `Required CI`, incluyendo la corrección del race de `schema_migrations`. PR #34 integró saga durable/idempotente, reconciliación PG hacia pinned INDEX, orphan debt tras safety window, exporter rollback que conserva escrituras posteriores a migración, y dump cifrado + restore aislado a segunda instancia con constraints/secretos/INDEX observation/garbage debt/rollback verificados. Merge #34 `27ccdcbf41bbce0265730754cc2ff32fd32dc140`; CI #255 pre-merge PASS completo; artefacto `postgres-recovery-evidence` digest `sha256:09622af6bd0d663b328ce28cb2955245db9fd995d93428e9a2fdeeb3a8c07df1`; CI post-merge #256 (`32946267360`) PASS completo y probe heredado más reciente #55 (`32946271700`) PASS. **Auditoría read-only posterior al merge confirma que `server.js` solo hace bootstrap/migrations cuando PG está enabled, mientras `server-core.js` todavía carga y guarda cuentas/control metadata desde JSON: el cutover de authority todavía no existe. Tampoco existen todavía PITR/WAL/KMS ni RPO/RTO productivos. No hay `[x]`.**
