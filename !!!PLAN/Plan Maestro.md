...
- **Estado general:** 🔴 `NO-GO` para lanzamiento público. Tareas 4.1, 4.2 y 5.1 quedan cerradas con evidencia. En 5.2 ya quedaron integrados el runtime PostgreSQL controlado/default-off, cutover/rollback fail-closed, worker durable de garbage/reconciliación, snapshot sellado/quarantine/final-delta exacto y aislamiento/rotación transaccional de OAuth/MFA. **El trabajo software-only exigible por 5.2 está cubierto; quedan infraestructura y operación productivas reales**: PostgreSQL, KMS/Secret Manager, backup cifrado + WAL/PITR/retención, restore independiente con RPO/RTO y cutover/rollback real. No iniciar tareas posteriores.
- **Último avance:** **PR #39 ya está integrado y todo el post-merge quedó verde.** #39 añadió keyring versionado y rotación transaccional de secretos PostgreSQL; el restore aislado real de CI probó que OAuth/MFA permanecen como ciphertext, que una DB/backup sin la key externa no puede autenticarse/descifrarse, que una key incorrecta o KMS ausente falla cerrado, que la rotación v7→v8 conserva el plaintext cambiando ciphertext/key_version y que un fallo de rotación revierte toda la transacción. Head pre-merge `a84a1652f06f7f52f0652369ba15b91ee2405c79`; CI #273 (`33009962358`) PASS completo y probe #68 (`33009962288`) PASS. Merge #39 `1a5cc387aef431cd5f5115ad537f55e80856fb08`; CI push post-merge #274 (`33010599812`) PASS completo incluido `Required CI`; probe #69 (`33010604236`) PASS. Recovery artifact id `9622353539`, digest `sha256:0053159a8e21be62e62e72a6996b5ca7baf97ee857db7655d4f827ecd99fc93c`. Antes, PR #37 había cerrado la mecánica final de cutover software con staging durable, bundles sellados, quarantine, final-delta refusal, binding a digest externo y READY atómico; merge `edad9e324132fa086ef729ef4faec574661578a9`, post-merge #272 PASS y probe #67 PASS. PR #38 fue cerrado **sin merge** por quedar supersedido por #37, evitando dos implementaciones conflictivas. **5.2 no se marca `[x]` porque la evidencia productiva real aún no existe.**
- **Próximo paso principal:** provisionar la infraestructura real de 5.2 por la ruta más corta compatible con los gates: PostgreSQL administrado + KMS/Secret Manager real + backups cifrados/PITR/retención + observabilidad; ejecutar restore independiente representativo y demostrar RPO <=15 min/RTO <=2 h; después realizar el maintenance-window cutover/rollback con los bundles y runbook ya implementados. No sustituir estos pasos por flags o pruebas sintéticas.
