...
- **Último avance:** M0-F de Tarea 5.1 ya dejó probado el límite real de delete cross-bot. El probe negativo confirmó que un transport bot miembro normal puede borrar su propio mensaje pero no el de otro bot. Después, en el mismo draft PR BeatGaler #25 `test(security): probe M0-F delete permission boundary`, el transport bot actual inició primero su sesión MTProto, fue añadido después a un vault privado como admin con `delete_messages`, la misma sesión aprendió el peer privado/access hash y ejecutó `channels.deleteMessages` sobre un mensaje reciente creado por otro transport bot: `cross_bot_delete_mtproto_proven=true`, `private_peer_learned_by_same_mtproto_session=true`, `public_vault_required=false` y `master_per_file_cleanup_used=false`. Un probe posterior sobre un mensaje real garantizado por el owner como >48 h y creado por otro bot llegó hasta `channels.deleteMessages` pero Telegram respondió `MESSAGE_DELETE_FORBIDDEN`; por decisión del RO, el borrado físico cross-bot >48 h queda como deuda futura de baja prioridad y **no bloquea 5.1** mientras el INDEX sea la autoridad y el garbage journal/GC se diseñe en 5.2. El cleanup correcto queda: INDEX nuevo autoritativo primero, delete oportunista después; un fallo de cleanup no revive el asset viejo ni revierte el INDEX.
- **Próximo paso principal:** continuar Tarea 5.1 con **aislamiento cross-vault/shared-bot** bajo la frontera temporary-auth ya probada; después escalabilidad/admission control, decidir si expiración server-side/natural sigue siendo gate, migrar el runtime productivo sin credenciales compartidas y cerrar discovery/hardening restante. **No iniciar Tarea 5.2 todavía.**
