...
- **Último avance:** decisión final del RO para el pool Direct: BeatGaler debe intentar **un transport bot distinto por vault activo** y operar con más bots disponibles que vaults activos para que compartir sea excepcional; sin embargo, si todos los bots están ocupados, **se permite que un mismo transport bot atienda más de un vault como fallback de capacidad**. El reparto conserva la política justa por carga: todos reciben 1 vault antes de que alguno reciba un 2.º, etc. Se acepta explícitamente que una temporary auth representa la identidad del bot y no se considera scoped criptográficamente a un solo vault; por ello el caso shared-bot tiene un riesgo residual cross-vault conocido y aceptado por el RO. No se rediseñará BeatGaler alrededor de exclusividad obligatoria 1 bot = 1 vault ni se exigirá un probe destinado a demostrar un aislamiento por vault que la credencial de identidad compartida no promete. Esta decisión no cierra 5.1 ni cambia el `NO-GO`.
- **Próximo paso principal:** continuar Tarea 5.1 con **escalabilidad/admission control y reglas operativas del pool bajo exclusividad preferida + shared-bot fallback**, después decidir si expiración server-side/natural sigue siendo gate, migrar el runtime productivo sin credenciales compartidas y cerrar discovery/hardening restante. **No iniciar Tarea 5.2 todavía.**
