...
- **Estado general:** 🔴 `NO-GO` para lanzamiento público. Tareas 4.1, 4.2 y 5.1 quedan cerradas con evidencia; 5.2 sigue abierta aunque el cutover software controlado/default-off ya quedó integrado, porque aún faltan infraestructura productiva real, KMS, PITR/WAL/retención, RPO/RTO demostrados, cutover/rollback productivo y trabajo durable restante de garbage/reconciliación. No iniciar tareas posteriores.
- **Último avance:** **PR #35 ya está integrado y todo el post-merge quedó verde.** #35 implementó autoridad explícita `json|postgres` default-off, runtime PostgreSQL durable para control-plane, virtualización acotada de los JSON legacy sin dual-write a disco en modo PG, durability barrier, cutover preparado con snapshot SHA exacto y rollback fail-closed que exporta el estado PostgreSQL actual. Durante CI se corrigieron dos causas reales y mínimas: teardown `DROP DATABASE ... WITH (FORCE)` que emitía `57P01` después de un PASS, y el restore verifier heredado que aún esperaba solo migrations `0001/0002` en vez de `0001/0002/0003`. Head pre-merge `5a567ca57e677f023f5e2343436c9e2f42369314`; CI pre-merge #263 (`32998574256`) PASS completo y probe #61 (`32998574221`) PASS. Merge #35 `d98181582a37a7c7a15ca7c1c047faac7ded6d63`; CI push post-merge #264 (`32999292323`) PASS completo, CI PR de integración #265 (`32999298396`) PASS y probe post-merge #62 (`32999298425`) PASS sobre el mismo SHA. Artefacto `postgres-recovery-evidence` id `9617859760`, digest `sha256:63b1acc4dc9caaaf35a3f36f471102a5a57d3569d7e5ee54a004726a37f1c4a6`. **Esto prueba cutover/rollback y restore en CI con dataset pequeño; no prueba KMS, PITR, RPO/RTO ni operación productiva. 5.2 no se marca `[x]`.**
- **Próximo paso principal:** completar dentro de 5.2 el worker durable de garbage/reconciliación y adversarial tests restantes, y cerrar el runbook/mecánica final de cutover (snapshot externo inmutable, quarantine de inválidos, write freeze/delta final, switch/rollback). El cierre de 5.2 exige además proveedor PostgreSQL real, KMS/Secret Manager real, backups cifrados, WAL/PITR/retención, restore independiente y RPO <=15 min/RTO <=2 h demostrados. No adelantar tareas posteriores.
...
- **Evidencia 5.2 — EN PROGRESO `[ 🟡 ]`:** arquitectura #29 integrada; #30 añadió schema PostgreSQL/control-plane + constraints + migration contract; #31 añadió migration runner, configuración PostgreSQL fail-closed y secret envelope AES-256-GCM; #32 añadió importador legacy idempotente, garbage journal repository y auth schema compatibility; #33 integró `pg@8.23.0`, bootstrap opt-in fail-closed y PostgreSQL 16 real dentro de `Required CI`, incluyendo la corrección del race de `schema_migrations`; #34 integró saga durable/idempotente, reconciliación hacia pinned INDEX, orphan debt, exporter de rollback y recovery drill. **PR #35 cerró el hueco de runtime auditado:** autoridad `json|postgres` explícita/default-off; PostgreSQL durable authority para control-plane; JSON legacy virtualizado solo como compatibilidad en modo PG sin disk dual-write; acknowledgement bloqueado hasta persistencia; cutover ligado a snapshot SHA exacto; rollback ciego rechazado y export de rollback desde el estado PG actual; OAuth/MFA hacen round-trip cifrado. Head #35 `5a567ca57e677f023f5e2343436c9e2f42369314`; CI pre-merge #263 (`32998574256`) PASS completo; probe #61 (`32998574221`) PASS; merge `d98181582a37a7c7a15ca7c1c047faac7ded6d63`; post-merge #264 (`32999292323`) PASS completo, #265 (`32999298396`) PASS y probe #62 (`32999298425`) PASS. Recovery artifact id `9617859760`, digest `sha256:63b1acc4dc9caaaf35a3f36f471102a5a57d3569d7e5ee54a004726a37f1c4a6`. **Pendiente antes de `[x]`: KMS/Secret Manager real, provider PostgreSQL productivo, backup cifrado + WAL/PITR/retención, restore independiente con RPO/RTO demostrados, mecánica final de cutover productivo y worker durable/adversarial restante.**
