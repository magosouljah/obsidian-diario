...
- **Estado general:** 🔴 `NO-GO` para lanzamiento público. Tareas 4.1, 4.2 y 5.1 quedan cerradas con evidencia; 5.2 está abierta y siguen pendientes P0/P1 posteriores, dependencias externas y gates globales de release.
- **Último avance:** **Tarea 5.2 iniciada y arquitectura aprobada por el Release Owner el 25 de agosto de 2026.** La auditoría read-only confirmó control-plane productivo todavía basado parcialmente en JSON/memoria y ausencia de PostgreSQL/garbage journal durable. Se abrió el draft PR BeatGaler #29 `docs(data): approve Task 5.2 durable architecture`, rama `task-5.2-data-architecture-docs`, head `ab071808a3dcfe5ce29fa5833d8b37cdfd0159b9`, con ADR, threat model y plan de migración/rollback. La decisión aprobada mantiene el pinned INDEX como única autoridad lógica de biblioteca, adopta PostgreSQL como autoridad durable del control-plane, envelope encryption con KEK/KMS externo para secretos recuperables, RPO objetivo <=15 min, RTO <=2 h y garbage/reconciliation durable donde fallos físicos nunca revierten INDEX ni resucitan assets. **No se tocó código productivo y 5.2 no se marca `[x]`.**
- **Próximo paso principal:** revisar el draft PR #29 y su CI, corregir únicamente evidencia/documentación si hace falta y, solo después de integrar el checkpoint arquitectónico, comenzar la implementación sensible de 5.2 conforme a los adversarial tests definidos. No adelantar tareas posteriores.
...
- **BeatGaler integración:** rama `integration-v0.8.0-alpha.1`; HEAD integrado tras 5.1 `d9ae76f42faee3a7207b9232b7421a0bec20b090`; versión `0.8.0-alpha.1`. La rama documental de 5.2 aún no está integrada.
...
- **Evidencia 5.2 — EN PROGRESO `[ 🟡 ]`:** auditoría read-only completada; RO aprobó PostgreSQL como control-plane durable sin desplazar al pinned INDEX; draft PR #29 añade `docs/ADR-0052-DURABLE-DATA-ARCHITECTURE.md`, `docs/THREAT-MODEL-0052-DATA-PERSISTENCE.md` y `docs/MIGRATION-0052-POSTGRES-ROLLBACK.md`; head `ab071808a3dcfe5ce29fa5833d8b37cdfd0159b9`. Pendientes: CI/review/integración del checkpoint documental y la implementación/pruebas de persistencia, migraciones, secrets, backup/restore, rollback, reconciliation y garbage journal exigidas por los gates. No hay `[x]` todavía.
