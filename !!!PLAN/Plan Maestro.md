...
- **Último avance:** M0-A de Tarea 5.1 ya tiene un probe aislado en PR BeatGaler #13, rama `task-5.1-temp-auth-binding-probe`, head `96af35e85481ff85d856dc22949bfb314ebedc3e`. El diff contra el baseline integrado es únicamente `scripts/telegram-temp-auth-binding-vector.mjs` + una línea de `package.json` que lo conecta a `test:regressions`. Con material totalmente sintético y sin red, el vector valida AES-IGE, serialización `bind_auth_key_inner` y derivación MTProto v1, y demuestra solo la frontera local: el binder puede construir el `encrypted_message` usando permanent auth material sin recibir bytes de la temporary auth key ni archivos del usuario. El self-test aislado produjo PASS. **No demuestra todavía** que Telegram acepte el split, una operación MTProto real, renovación, 1.9 GB ni integración por plataforma. CI #109 está en progreso.
- **Próximo paso principal:** completar CI de M0-A y después construir el siguiente probe de red, todavía aislado: dispositivo genera temporary auth key, lado controlado construye el binding sin entregar permanent bot auth, dispositivo envía `auth.bindTempAuthKey` directamente a Telegram y ejecuta una operación MTProto directa. Antes de usar la stack existente como supuesto, hay que verificar si `@mtcute/web`/GramJS exponen la creación/importación de temporary auth keys o si M0 requiere una capa MTProto de bajo nivel aislada. Después: renovación, operación larga, 1.9 GB, Windows/macOS/Web pura, delete cross-bot, cross-vault/shared-bot y revisión independiente.
...
- **Evidencia 5.1 (en progreso):** PR #11 `security: define Task 5.1 trust boundaries`; head inicial `5cdcfcecccea63a31adc5eaf66416929c0fbb95a`; CI #103 PASS 6/6. Head documental corregido `bb162c01c80e21a264c4022c9c682a90c14fbb98`; CI #108 en progreso al último chequeo, con Web/shared, Supply chain, Windows y macOS arm64 ya PASS y macOS x86_64 aún ejecutándose. PR #12 `test(security): probe Telegram admin-rights churn`, head `2b8904880dfeaa57b970674a79abcb181161af0a`, CI #105 PASS 6/6; prueba riesgo de churn, no una cifra universal. PR #13 `test(security): prove M0 temp-auth binding boundary`, draft, head `96af35e85481ff85d856dc22949bfb314ebedc3e`; self-test aislado PASS; CI #109 en progreso. Ningún checkbox 5.1 queda cerrado todavía.
