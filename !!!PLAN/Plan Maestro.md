...
- **Estado general:** 🔴 `NO-GO` para lanzamiento público. Tareas 4.1, 4.2 y 5.1 quedan cerradas con evidencia; 5.2 sigue abierta aunque el cutover software controlado/default-off y el worker durable de garbage/reconciliación ya quedaron integrados, porque aún faltan infraestructura productiva real, KMS, PITR/WAL/retención, RPO/RTO demostrados y cutover/rollback productivo. No iniciar tareas posteriores.
- **Último avance:** **PR #36 ya está integrado y todo el post-merge quedó verde.** #36 añadió el worker durable de garbage/reconciliación sobre las primitivas existentes: lease reclamable con `SKIP LOCKED`, retry/backoff durable, reconciliación idempotente desde pinned INDEX hacia PostgreSQL y cleanup físico inyectable sin acoplar el worker al transporte Direct. El adversarial PostgreSQL cubre el caso crítico “efecto externo ocurre → crash antes de marcar `done` → lease vence → otro worker recupera → objeto ya ausente se trata idempotentemente”, sin reescribir INDEX ni resucitar tombstones. Head pre-merge `97ff8a5ab85dd49905faf2dae32baa4ca41bc893`; CI pre-merge #266 (`33001989737`) PASS completo y probe #63 (`33001989631`) PASS. Merge #36 `97c120f7605c36d0862407d9d53c821262ce1a64`; CI push post-merge #267 (`33002946139`) PASS completo incluido `Required CI`; probe post-merge #64 (`33002953220`) PASS sobre el mismo SHA. Artefacto `postgres-recovery-evidence` id `9619336011`, digest `sha256:dc4587dfc4c634145594fddbaf01eba4e70fad2da37cbb81e8219ff210fc0cbc`. **Esto cierra el hueco de worker durable/adversarial identificado, pero no prueba KMS, PITR, RPO/RTO ni operación productiva. 5.2 no se marca `[x]`.**
- **Próximo paso principal:** cerrar dentro de 5.2 la mecánica final de cutover productivo (snapshot externo inmutable, quarantine de inválidos, write freeze/delta final, switch/rollback) y preparar la integración con infraestructura real. El cierre de 5.2 exige además proveedor PostgreSQL real, KMS/Secret Manager real, backups cifrados, WAL/PITR/retención, restore independiente y RPO <=15 min/RTO <=2 h demostrados. No adelantar tareas posteriores.
...
- **Evidencia 5.2 — EN PROGRESO `[ 🟡 ]`:** arquitectura #29 integrada; #30 añadió schema PostgreSQL/control-plane + constraints + migration contract; #31 añadió migration runner, configuración PostgreSQL fail-closed y secret envelope AES-256-GCM; #32 añadió importador legacy idempotente, garbage journal repository y auth schema compatibility; #33 integró `pg@8.23.0`, bootstrap opt-in fail-closed y PostgreSQL 16 real dentro de `Required CI`, incluyendo la corrección del race de `schema_migrations`; #34 integró saga durable/idempotente, reconciliación hacia pinned INDEX, orphan debt, exporter de rollback y recovery drill; #35 integró autoridad runtime `json|postgres` default-off, PostgreSQL durable authority para control-plane, JSON legacy virtualizado sin disk dual-write, durability barrier, cutover ligado a snapshot SHA exacto y rollback desde estado PG actual. **PR #36 cerró el hueco de worker durable/adversarial:** lease recuperable, `SKIP LOCKED`, retry/backoff, cleanup idempotente y reconciliación periódica INDEX→PG; el caso crash-after-external-effect fue probado en PostgreSQL real y el recovery drill siguió verde. Head #36 `97ff8a5ab85dd49905faf2dae32baa4ca41bc893`; CI pre-merge #266 (`33001989737`) PASS completo; probe #63 (`33001989631`) PASS; merge `97c120f7605c36d0862407d9d53c821262ce1a64`; post-merge #267 (`33002946139`) PASS completo y probe #64 (`33002953220`) PASS. Recovery artifact id `9619336011`, digest `sha256:dc4587dfc4c634145594fddbaf01eba4e70fad2da37cbb81e8219ff210fc0cbc`. **Pendiente antes de `[x]`: KMS/Secret Manager real, provider PostgreSQL productivo, backup cifrado + WAL/PITR/retención, restore independiente con RPO/RTO demostrados y mecánica final de cutover productivo.**
