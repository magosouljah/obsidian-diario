...
- **Último avance:** Tarea 5.1 sigue `DRAFT/NO-GO`. PR BeatGaler #11 mantiene la restricción obligatoria de archivos **directamente dispositivo ↔ Telegram** y fue corregido para sacar del diseño principal el grant/revoke dinámico de permisos. PR #12 probó que permission churn frecuente puede disparar `FLOOD_WAIT` e impedir incluso la restauración inmediata: una primera corrida logró 80 cambios sin flood; una segunda corrida poco después, a 250 ms, recibió `FLOOD_WAIT 533s` después de 20 cambios de esa corrida y el restore inmediato también recibió `FLOOD`. La actividad acumulada impide afirmar que “el límite sea 20”. `pin_messages` y, si delete cross-bot lo exige, `delete_messages` se evaluarán como baseline mínimo estable. CI #103 de la versión previa de #11 y CI #105 de #12 terminaron PASS completo.
- **Próximo paso principal:** M0 aislado de **Telegram Ephemeral Direct**. Probar primero la frontera protocolaria permanent-side/temp-side: permanent bot auth nunca llega al cliente; el dispositivo genera/usa temporary auth key; el lado controlado construye el binding sin recibir la temp key completa; el dispositivo ejecuta `auth.bindTempAuthKey` y luego una operación MTProto directamente contra Telegram. Después: renovación, operación larga, 1.9 GB, Windows/macOS/Web pura, delete cross-bot, cross-vault/shared-bot y revisión independiente.
...
- **Evidencia 5.1 (en progreso):** PR #11 `security: define Task 5.1 trust boundaries`; head inicial auditado `5cdcfcecccea63a31adc5eaf66416929c0fbb95a`; CI PR #103 (`32755046082`) PASS 6/6 incluido `Required CI`; actualización documental posterior hasta `bb162c01c80e21a264c4022c9c682a90c14fbb98` incorpora decisión de permisos baseline y M0 split binding, con CI nuevo por verificar. PR #12 `test(security): probe Telegram admin-rights churn`, head `2b8904880dfeaa57b970674a79abcb181161af0a`, CI PR #105 (`32758345390`) PASS 6/6; su resultado demuestra riesgo de churn pero no una cifra universal de rate limit. Ningún checkbox 5.1 queda cerrado todavía.
