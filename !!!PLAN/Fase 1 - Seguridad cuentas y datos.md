...
- La decisión D8 de recent reauth existe, pero falta seam productiva consumible por Settings/Trash; owner BBB136.

### HARD / ACTIVE BLOCKERS para alpha

1. **F2/12.1:** software lineage integrada; public runtime proof exacto post-`aa445095...` sigue pendiente. Owner AAA137 READ-ONLY para evidencia/clasificación.
2. **F0/0.9 security P1:** #89 stale-base + current F0 audit FAILURE; owner WOZ140 para diagnosis/refresh/revalidation/integration.
3. **F2/13.2 durable Review:** `BLOCKED_WRITE_SURFACE / UNASSIGNED`; factual gap remains.
4. **F1/D8 product seam → F2/15.1:** owner BBB136 expone seam mínima recent-reauth; después debe volver Trash strong confirmation + durable purge.
...
`NIGHT-AAA-136`, `NIGHT-BBB-135` y `NIGHT-WOZ-139` no produjeron RESULTADO DEL TURNO verificable. JOBS CYCLE141 recalculó desde cero y mantiene primero los tres reducers factuales/productivos de mayor impacto: 12.1 runtime, #89 y recent-reauth. Después se reemitirá 1.7 para clasificación `MUST_CLOSE / RO_EXCLUDE_CANDIDATE / RELEASE_ONLY_EXTERNAL` antes de 1.8.
