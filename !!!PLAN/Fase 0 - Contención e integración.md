...
En la situación actual, la **Tarea 5.2 sigue siendo el frente técnico principal** y Tareas 1.2/2.2 continúan como trabajo paralelo autorizado. Dentro de 5.2 pueden separarse subtracks externos independientes —por ejemplo secrets/KMS/IAM y RDS/PITR/restore— sin fingir que auditorías o preparación software sustituyen la evidencia productiva real exigida por el gate. Las asignaciones transitorias de cada oleada viven en Issue #41, no se duplican aquí salvo que cambien estado, evidencia o un gate.
...
- [ 🟡 ] Aprobar una persistencia transaccional durable con migrations, constraints, backup/restore y rollback como requisito de producción. **Arquitectura aprobada por RO el 25 de agosto de 2026; el software está integrado hasta PR #42 y WAVE 3 confirma que PostgreSQL ya es la autoridad productiva. La durabilidad sobrevivió un reinicio controlado y existe rollback dry-run desde el estado PostgreSQL actual. 5.2 continúa `[ 🟡 ] / NO-GO` hasta cerrar los cuatro criterios de salida congelados.**
- [ 🟡 ] **DECISIÓN APROBADA:** PostgreSQL será la autoridad durable del control-plane para cuentas, sesiones, providers, MFA, entitlements, vault metadata, Direct leases/operations, jobs, auditoría, garbage journal e INDEX observations/reconciliation. El INDEX fijado conserva la autoridad lógica de beats, trash y tombstones; PostgreSQL nunca se convierte en manifest authority de la biblioteca. **Estado productivo WAVE 3: esta autoridad PostgreSQL ya está activa.**
- [ 🟡 ] Definir cifrado de secretos, migraciones, backup, RPO/RTO propuestos y rollback. **Definido e implementado en software/CI:** AES-256-GCM autenticado, key_version, keyring multiversión, rotación PostgreSQL transaccional, importer/cutover/rollback, restore aislado y provider AWS Secrets Manager con `AWSCURRENT`/payload versionado/fail-closed; DB/backup sin key externa no expone plaintext OAuth/MFA en el adversarial. **Estado productivo WAVE 3:** rollback dry-run desde CURRENT PostgreSQL ya está demostrado; los únicos faltantes de este frente son aceptación independiente, restore representativo/RPO-RTO y rotación productiva multiversión según los cuatro criterios siguientes.
- [ 🟡 ] Definir reconciliación Telegram/index y garbage journal. **Definido e implementado en software/CI:** INDEX gana; PG stale se repara hacia INDEX; missing referenced object alerta sin reescribir verdad; unreferenced object pasa a orphan candidate tras safety window; cleanup físico es post-commit durable; `MESSAGE_DELETE_FORBIDDEN` nunca revierte INDEX ni resucita contenido; worker durable usa leases recuperables, `SKIP LOCKED`, retry/backoff y recovery crash-after-external-effect. **Pendiente solo la evidencia operativa incluida en los cuatro criterios WAVE 3.**

**Estado/evidencia 5.2 — EN PROGRESO:**
- **WAVE 3 — estado productivo confirmado en Issue #41:** `integration-v0.8.0-alpha.1@a968122127c584b5557b25e70a21eb64f75b3c0e` sigue siendo la base integrada; PostgreSQL ya es autoridad productiva; el estado del control-plane sobrevivió un restart controlado preservando marker, snapshot SHA y fingerprints; el proceso reiniciado volvió a servir únicamente después de pasar el barrier PostgreSQL fail-closed.
- **Rollback actual:** WOZ ejecutó un `READ_ONLY_ROLLBACK_DRY_RUN` contra la autoridad PostgreSQL viva y actual. El export/manifest se construyó desde CURRENT PostgreSQL, no desde JSON pre-cutover; no se escribieron JSON a disco, no se cambió `control_plane_cutovers`, no se imprimieron secretos y la autoridad viva siguió siendo PostgreSQL. **Esto aún no es un rollback productivo comprometido y requiere aceptación independiente.**
- **Cierre congelado — quedan exactamente cuatro criterios:** **(1)** aceptación independiente de la evidencia actual de rollback; **(2)** restore independiente representativo con **RPO <=15 min / RTO <=2 h** medidos; **(3)** evidencia productiva de rotación multiversión; **(4)** observabilidad + alertas + on-call + rollback authority. **No añadir otros criterios de salida dentro de 5.2 sin nueva decisión explícita de WOZ/RO.**
- **No repetir:** cutover, import JSON, migrations ni restart de durabilidad ya demostrados. Cualquier repetición requiere una nueva razón técnica explícita.
...
- **PR #42 cerró el finding de base64 permisivo registrado en Issue #41:** `parseAwsEnvelopeKeyringSecret` ahora exige round-trip canónico antes de aceptar una key AWS, mantiene el requisito exacto de 32 bytes y prueba negativos para caracteres inválidos, padding inválido y trailing data. Head `718efd9073144eb2fd89bfe2459ed2ac8996b079`; merge `a968122127c584b5557b25e70a21eb64f75b3c0e`; post-merge `Test - Desktop Portability` run `33118368302` PASS completo en PostgreSQL live/recovery, Web/shared, Supply Chain, Windows, macOS arm64/x86_64 y `Required CI`.
- **Wave 1 multi-cuenta, auditorías read-only — registro histórico:** AAA revisó production-readiness de Secrets Manager/KMS/IAM y BBB RDS/PITR/restore/RPO-RTO. Ambos concluyeron que no existía un nuevo blocker software que justificara retrasar provisioning y dejaron checklists concretos en Issue #41. Ese estado queda superado por la evidencia productiva posterior de WAVE 3 y no añade criterios actuales de salida.
...
- **Pendiente antes de cerrar 5.2 — SOLO:** **(1)** aceptación independiente de la evidencia actual de rollback; **(2)** restore independiente representativo con RPO <=15 min/RTO <=2 h medidos; **(3)** evidencia productiva de rotación multiversión; **(4)** observabilidad, alertas, on-call y rollback authority. **5.2 permanece `[ 🟡 ]`; NO-GO.**

**Dependencias:** contención terminada.  
**Evidencia:** ADR/threat model/migration plan + PRs #29–#42 + CI PostgreSQL real/recovery + cutover/rollback software + worker durable/adversarial + secret isolation/key rotation + AWS Secrets Manager provider software boundary + strict base64 hardening + evidencia productiva WAVE 3 de autoridad PostgreSQL, restart durable y rollback dry-run desde CURRENT PG.  
**Gate de salida:** arquitectura aprobada, control-plane operando realmente sobre persistencia durable y los **cuatro criterios WAVE 3** satisfechos con evidencia independiente/productiva conforme a `Gates - Publicación y contingencias.md`. **NO SATISFECHO todavía; no marcar `[x]`.**