...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. M0-A demostró la frontera criptográfica sintética, M0-B0 confirmó el seam real de mtcute, M0-B1 probó el split network permanent-side/temp-side, M0-B2 probó identidad bot + RPC directo con permanent bot credentials exclusivamente del lado controlado, M0-C probó renovación proactiva A→B y recovery tras pérdida/expiración simulada, M0-D probó transferencia directa exacta de 1.9 GB atravesando A→B bajo una sola `file_id`, M0-E1 probó el mismo B2 live en Windows x86_64, macOS arm64 y macOS x86_64, M0-E2 probó Web pura dentro de Chrome + Web Worker real con credenciales permanentes fuera del browser y M0-F probó delete propio/cross-bot reciente con el transport bot actual aprendiendo un vault privado desde una sesión MTProto ya activa. Esta subparte sigue `[ 🟡 ]`: no se ha sustituido todavía la credencial compartida en el runtime Web/Desktop, aún faltan aislamiento/escalabilidad, decisión explícita sobre expiración server-side/natural y migración productiva. El delete físico cross-bot >48 h queda como deuda futura de GC de baja prioridad y no bloquea 5.1 mientras INDEX sea la autoridad.
...
- Siguiente subgate 5.1: **aislamiento cross-vault/shared-bot** bajo temporary auth; después escalabilidad/admission control, decidir explícitamente si expiración server-side/natural sigue siendo requisito, migración del runtime productivo sin credenciales compartidas y hardening restante.
- **No se cambió** transporte runtime productivo, Bot API productivo, Offline, YouTube, Pinterest, UI, token rotation/revoke, datos de usuario, CSP, CORS, cookies, permisos Tauri ni parser ID3.
