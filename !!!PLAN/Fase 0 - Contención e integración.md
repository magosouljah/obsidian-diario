...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. M0-A demostró la frontera criptográfica sintética, M0-B0 confirmó el seam real de mtcute, M0-B1 probó el split network permanent-side/temp-side, M0-B2 probó identidad bot + RPC directo con permanent bot credentials exclusivamente del lado controlado, M0-C probó renovación proactiva A→B y recovery tras pérdida/expiración simulada, M0-D probó transferencia directa exacta de 1.9 GB atravesando A→B bajo una sola `file_id`, y M0-E1 ya probó el mismo B2 live en Windows x86_64, macOS arm64 y macOS x86_64. Esta subparte sigue `[ 🟡 ]`: Web pura todavía no está probada bajo la nueva frontera, no se ha sustituido la credencial compartida en runtime Web/Desktop, no se ha probado expiración temprana inducida por Telegram ni expiry natural, y aún faltan aislamiento/escalabilidad.
...
- M0-E1: draft PR #23 `test(security): prove temp auth across Windows and macOS`, rama `task-5.1-temp-auth-cross-platform-probe`, head `8ea250cf7b71dda6d09fd408deb473a9151e7d2e`, apilado sobre #18. El diff neto frente a M0-D cambia únicamente `.github/workflows/test-desktop-portability.yml` para añadir un job branch-scoped de prueba; no toca runtime. Workflow #173 (`32827228117`): los tres jobs live **Windows x86_64**, **macOS arm64** y **macOS x86_64** terminaron PASS ejecutando sin cambios el probe B2 contra production DC 1. En los tres casos: `bot_identity_proven=true`, `network_bind_proven=true`, `direct_mtproto_operation_proven=true`, `permanent_auth_reaches_client=false`, `bot_token_reaches_client=false`, `api_hash_reaches_client=false`, `galer_file_bytes=false`, `vault_used=false`, `token_rotation_or_revoke=false`. El propio probe registra `production_runtime_changed=false` y `web_browser_proven=false`. Una primera edición del workflow falló antes de crear jobs por sintaxis YAML del harness; se corrigió sin cambiar producto y el head final es el que produce los tres PASS live. El CI normal #173 sigue en progreso al registrar esta evidencia; no se afirma todavía `Required CI` PASS.
- Auditoría Web posterior a M0-E1: el runtime Web actual **todavía no satisface la frontera objetivo**. `webTransportSession.ts` descifra dentro del navegador `bot_token`, API id/hash; `webTransportWorkerProtocol.ts` los incluye en `initialize`; `webTransport.worker.ts` construye `TelegramClient` con API id/hash y hace `start({ botToken })`. Por tanto, un script Node no cuenta como evidencia Web. El siguiente probe debe ejecutarse en navegador/Worker real y demostrar temp auth directa sin entregar credenciales permanentes al browser.
- Siguiente subgate 5.1: **M0-E2 Web pura en navegador/Worker real**; después probar delete propio/cross-bot, aislamiento cross-vault/shared-bot, escalabilidad/admission control, expiración server-side/natural si sigue requerida y la migración del runtime productivo sin credenciales compartidas.
