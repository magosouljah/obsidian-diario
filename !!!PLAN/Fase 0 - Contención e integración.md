...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. M0-A demostró la frontera criptográfica sintética, M0-B0 confirmó el seam real de mtcute, M0-B1 probó el split network permanent-side/temp-side, M0-B2 probó identidad bot + RPC directo con permanent bot credentials exclusivamente del lado controlado y M0-C ya probó renovación proactiva A→B, retención local de la key anterior como secondary y recovery tras pérdida/expiración simulada mediante una fresh key C. Esta subparte sigue `[ 🟡 ]`: M0-C no demuestra expiración temprana inducida por Telegram ni una espera a expiry natural, tampoco una transferencia larga real; siguen faltando data plane 1.9 GB y reemplazar de verdad las credenciales compartidas en el runtime Web/Desktop.
...
- Siguiente gate M0-D: demostrar data plane directo hasta **1.9 GB** sin relay de Galer Cloud. Preferir una transferencia real que atraviese una renovación de temp key; si no la atraviesa, la cobertura de operación larga real seguirá abierta como subgate independiente.
- Después siguen Windows/macOS/Web pura, delete cross-bot, cross-vault/shared-bot, escalabilidad/admission control, expiración server-side/natural si continúa requerida y migración del runtime productivo sin credenciales compartidas.
