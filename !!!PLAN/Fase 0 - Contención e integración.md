...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. M0-A demostró la frontera criptográfica sintética, M0-B0 confirmó el seam real de mtcute, M0-B1 probó el split network permanent-side/temp-side con un bind aceptado por Telegram TEST DC y M0-B2 ya tiene harness aislado/validado para identidad bot, pero su ejecución live sigue pendiente por secretos dedicados ausentes en Actions. Falta demostrar identidad bot y operación directa autorizada antes de considerar satisfecha esta subparte.
...
- M0-B2 harness: draft PR #16 `test(security): prepare M0-B2 bot temp-auth proof`, rama `task-5.1-temp-auth-bot-probe`, head `43c43353dce5a025340bd142222a57583ba4a609`, apilado sobre #15. Cambia solo `scripts/telegram-temp-auth-bot-live.mjs` y `.github/workflows/probe-task-5.1-temp-auth-bot.yml`. El binder parent es el único que recibe los secretos dedicados y autoriza la permanent key mediante `auth.importBotAuthorization`; el cliente se lanza con API id/hash/token eliminados de su environment, genera la temp key, hace el bind directo y exige que `users.getUsers(inputUserSelf)` coincida con la identidad bot autorizada por binder. Workflow B2 #1: syntax/harness PASS, live step SKIPPED porque `BEATGALER_M0_B2_API_ID`, `BEATGALER_M0_B2_API_HASH` y `BEATGALER_M0_B2_BOT_TOKEN` no están configurados; output explícito `B2_LIVE_PROOF=NOT_RUN`. CI normal #128 PASS 6/6 incluido `Required CI`. **Esto no prueba B2:** `bot_identity_proven=false` y `direct_mtproto_operation_proven=false` siguen vigentes.
- Siguiente gate M0-B2 live: configurar esos tres secretos únicamente en GitHub Actions y reejecutar el probe. No compartir valores en chat, PR, logs ni código. Solo un PASS con identidad coincidente permite avanzar a renovación/expiración/operación larga.
