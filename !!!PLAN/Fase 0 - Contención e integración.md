...
Desde el 27 de agosto de 2026 el equipo operativo se divide en **cuatro roles** según `Equipo multi-IA - Roles y coordinación.md`: `ORION` es dueño de `!!!PLAN` y coordinador normal de AAA/BBB; `ATLAS` es jefe técnico e integrador; `AAA` y `BBB` son ayudantes ejecutores. ORION mantiene el plan limpio, detecta desincronización, consulta Issue #41, asigna/reasigna automáticamente trabajo independiente a AAA/BBB y entrega a ATLAS el siguiente frente técnico permitido. ORION no modifica BeatGaler ni infraestructura; sus únicas escrituras de archivos de repositorio son dentro de `!!!PLAN`.

ATLAS conserva la autoridad técnica: arquitectura, decisiones de implementación, código, infraestructura, aceptación/rechazo de findings e integración. AAA/BBB no cambian gates ni marcan `[x]`; entregan evidencia/handoff. BeatGaler Issue #41 es el `AI Coordination Inbox` para asignaciones, blockers y handoffs entre cuentas.
...
- [ 🟡 ] Aprobar una persistencia transaccional durable con migrations, constraints, backup/restore y rollback como requisito de producción. **Arquitectura aprobada por RO el 25 de agosto de 2026; el mecanismo software de cutover/rollback, recuperación, provider de secretos y hardening de parsing está integrado hasta PR #42. Falta ejecutar y evidenciar la infraestructura productiva real.**
...
- **Wave 1 multi-cuenta, auditorías read-only:** AAA revisó production-readiness de Secrets Manager/KMS/IAM y BBB RDS/PITR/restore/RPO-RTO. Ambos concluyeron que no existe un nuevo blocker software que justifique retrasar el provisioning; dejaron checklists concretos en Issue #41. Esto **no** constituye evidencia productiva ni cambia el gate. BBB además señaló que el contrato actual usa el mismo `DATABASE_URL` para migrations/DDL y CRUD runtime; ATLAS debe aceptar explícitamente ese modelo de privilegios o decidir una separación antes del uso productivo, sin confundirlo con un fallo ya demostrado.
