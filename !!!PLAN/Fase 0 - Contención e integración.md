...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. La candidata es MTProto con temporary authorization keys de bot renovables y RAM-only; M0-A demostró la frontera criptográfica sintética, M0-B0 confirmó el seam real de mtcute y M0-B1 ya probó el split network permanent-side/temp-side con un bind aceptado por Telegram TEST DC. Falta demostrar identidad bot y operación directa autorizada antes de considerar satisfecha esta subparte.
...
- Siguiente gate M0-B2: demostrar una identidad bot autorizada y una operación MTProto directa usando la temporary auth key mientras permanent bot auth/credenciales permanecen solo del lado controlado. No crear/migrar vaults reales ni transferir archivos de usuario en B2; secretos únicamente server-side/CI, sin logs ni entrega al cliente.
