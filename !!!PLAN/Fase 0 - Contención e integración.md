...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. M0-A demostró la frontera criptográfica sintética, M0-B0 confirmó el seam real de mtcute, M0-B1 probó el split network permanent-side/temp-side, M0-B2 probó identidad bot + RPC directo con permanent bot credentials exclusivamente del lado controlado, M0-C probó renovación proactiva A→B y recovery tras pérdida/expiración simulada, M0-D probó transferencia directa exacta de 1.9 GB atravesando A→B bajo una sola `file_id`, M0-E1 probó el mismo B2 live en Windows x86_64, macOS arm64 y macOS x86_64, y M0-E2 ya probó Web pura dentro de Chrome + Web Worker real con credenciales permanentes fuera del browser. Esta subparte sigue `[ 🟡 ]`: no se ha sustituido todavía la credencial compartida en el runtime Web/Desktop, no se ha probado expiración temprana inducida por Telegram ni expiry natural, y aún faltan delete propio/cross-bot, aislamiento/escalabilidad y migración productiva.
