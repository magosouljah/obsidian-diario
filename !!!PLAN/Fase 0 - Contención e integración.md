...
- [ 🟡 ] Aprobar una persistencia transaccional durable con migrations, constraints, backup/restore y rollback como requisito de producción. **Arquitectura aprobada por RO el 25 de agosto de 2026; cutover software controlado/default-off ya integrado hasta PR #35; faltan cutover productivo y evidencia de infraestructura real.**
...
- **Pendiente antes de cerrar 5.2:** proveedor PostgreSQL productivo y KMS/Secret Manager real; backups cifrados, WAL/PITR y retención; restore independiente que demuestre RPO <=15 min/RTO <=2 h; mecánica final de cutover productivo con snapshot externo inmutable, quarantine, write freeze/delta final/switch y rollback real; worker durable de garbage/reconciliation con retries/leases y adversarial tests restantes; evidencia stolen-DB/backup-sin-KMS y key rotation. **5.2 permanece `[ 🟡 ]`; NO-GO.**

**Dependencias:** contención terminada.  
**Evidencia:** ADR/threat model/migration plan + PRs #29–#35 + CI PostgreSQL real/recovery + cutover/rollback software + post-merge integrado verde.  
