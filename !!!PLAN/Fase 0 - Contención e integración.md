...
- **M0-G — escalabilidad/admission control del pool:** draft PR #26 `feat(direct): prove and enforce Task 5.1 pool admission`, rama `task-5.1-pool-scalability-admission`, head `f39578ab0afe40448566087edfdbb62a2c80967a`. El RO fija máximo de **4 vaults activos por bot**. El probe de caracterización `32891545205` PASS demostró con 80 bots y 320 leases sintéticas que el fair scheduler conserva cuatro rondas exactas y que 16 workers concurrentes terminan con carga 4/4/.../4, IDs únicos y estado válido; también confirmó que antes del cambio una 321.ª lease podía elevar un bot a 5. El admission wrapper conserva el scheduler existente, distingue `exclusive` vs `shared_fallback`, solo activa sharing cuando `freeBots=0` y, cuando todos los bots elegibles llegan a 4, envía nuevas sesiones a una waitlist acotada en memoria (default 320, timeout 30 s, configurables) con rechazo explícito al llenarse o expirar. Expone `free_bots`, `shared_bots`, `shared_leases`, capacidad, waitlist y contadores. Workflow dedicado final `32891823812` PASS después de corregir que `.env` se cargue antes del wrapper. Con 80 bots, la capacidad configurada es 320 vaults activos simultáneos; bajo la hipótesis RO de 3% activos equivale a ~10,666 usuarios totales antes de alcanzar esa demanda esperada. Esto es planeación, no SLA ni sustituto del gate global de carga 2×. No se usó Telegram real, datos/vaults reales ni token rotation/revoke.
- **M0-G CI final:** CI normal #192 (`32891830172`) del mismo head terminó **PASS 6/6**: Web/shared, Supply chain, Windows, macOS arm64, macOS x86_64 y `Required CI`. Con esto queda cerrada la evidencia técnica del subgate de pool/admission, sin cerrar Tarea 5.1 completa.
- **M0-H — expiración natural/server-side temporary-auth probada live:** draft PR #27 `test(security): prove natural temp-auth expiry`, rama `task-5.1-temp-auth-natural-expiry-probe`, head `4ddd6052f0fee7e2b6c82e3f2ebb86610b4c1f3e`, apilado sobre M0-C. Temp A se solicitó con TTL de 60 s y funcionó antes de `expires_at`. Sin reset/fault injection local, sin revoke ni rotation y conservando exactamente esa A, después del wall-clock expiry Telegram rechazó dos RPC consecutivas con `400:TIMEOUT`; a continuación una Temp B fresca fue generada/bindada y volvió a operar con la misma identidad bot. Workflow dedicado `32895996417`, job `97958708150`, PASS; artefacto `natural-expiry-evidence` digest `sha256:39d0159d6c8cc44a6d06219080aa1b9afc2883033ebd6cf92e553165856cc4a2`. CI normal #193 (`32896306047`) PASS 6/6: Web/shared, Supply chain, Windows, macOS arm64, macOS x86_64 y `Required CI`. No se tocaron runtime productivo, vaults/datos reales ni token rotation/revoke. La expiración natural deja de ser incógnita pendiente de 5.1.
- Siguiente subgate 5.1: **migrar el runtime productivo Web/Desktop al modelo temporary-auth sin credenciales permanentes compartidas**, manteniendo data plane directo y rollback; después discovery/hardening restante y revisión independiente.
