...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. La candidata es MTProto con temporary authorization keys de bot renovables y RAM-only; M0-A demostró la frontera criptográfica sintética y M0-B0 confirmó el seam real de mtcute, pero el split network permanent-side/temp-side sigue siendo hipótesis hasta un bind aceptado por Telegram.
...
- Siguiente gate M0-B1: en un probe aislado, generar permanent y temporary MTProto auth keys en lados separados; mantener permanent auth solo en binder/controlado; cliente conserva temp key y envía solo metadata mínima; binder devuelve únicamente el envelope; cliente ejecuta `auth.bindTempAuthKey` **directamente contra Telegram**. Preferir test DC para reducir impacto. Solo una respuesta exitosa de Telegram permite cambiar `network_bind_proven=true`; esto todavía no prueba identidad bot ni operación de archivos.
- Después siguen identidad bot/operación MTProto directa, renovación, expiración anticipada, operación larga, **1.9 GB**, Windows/macOS/Web pura, delete cross-bot, cross-vault/shared-bot y escalabilidad/admission control.
