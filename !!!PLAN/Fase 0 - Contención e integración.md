...
- [ 🟡 ] Sustituir credenciales Telegram en cliente por acceso temporal seguro sin romper el data plane directo. **Decisión de diseño en progreso:** Galer Cloud nunca será relay de archivos. La candidata es MTProto con temporary authorization keys de bot renovables y RAM-only; M0-A demostró solamente la frontera criptográfica sintética del binding. El split real permanent-side/temp-side sigue siendo hipótesis hasta un bind de red aceptado por Telegram.
...
- La especificación oficial de Telegram confirma una separación protocolariamente plausible: el `encrypted_message` de binding se cifra con la permanent auth key, mientras `auth.bindTempAuthKey` se invoca usando la temporary auth key. Telegram no documenta una API de “delegación remota”; por tanto el split servidor/cliente sigue siendo hipótesis hasta prueba end-to-end.
...
- M0-A: PR #13 `test(security): prove M0 temp-auth binding boundary`, draft, rama `task-5.1-temp-auth-binding-probe`, head `96af35e85481ff85d856dc22949bfb314ebedc3e`. Parte del baseline integrado exacto `f6d1f998bd63589ec2ddad7ee4d5818e9b85f016`; diff = 2 archivos (`scripts/telegram-temp-auth-binding-vector.mjs` y una línea de `package.json`). No usa red, bot token, API hash, auth keys reales ni vault. Valida un vector AES-IGE publicado y el envelope MTProto v1 con material sintético. El self-test aislado dio `PASS M0-A temp-auth binding vector: permanent-side envelope can be built without temp-key bytes or file relay`. Su API de binder no acepta bytes de temp auth key. Esto demuestra solamente una frontera de implementación local; `network_bind_proven=false` y `direct_mtproto_operation_proven=false`. CI #109 ya pasó Web/shared, Supply chain, Windows y macOS arm64; macOS x86_64 sigue en progreso, por lo que `Required CI` aún no es final. Los logs Web/shared muestran la ejecución real del nuevo vector y sus flags negativos de network/direct proof.
- Auditoría read-only de mtcute para M0-B: `doAuthorization(connection, crypto, expiresIn)` ya implementa creación de temporary auth keys y la variante PFS del handshake; `SessionConnection` implementa TTL/refresh, pero antes exige la permanent auth key y luego hace el bind desde la misma conexión. Por eso activar `usePfs` sin cambios **no satisface 5.1**: mantendría permanent auth en el cliente. BeatGaler Web ya usa `MemoryStorage`, por lo que RAM-only no es el bloqueo. `MtClientOptions` exige `apiHash`, pero el constructor de `MtClient` no pasa `apiHash` a `NetworkManager`; solo `apiId`. Esto hace plausible un adapter/probe de red de nivel inferior sin `api_hash` cliente, pero aún no está probado.
- Siguiente gate M0-B: reutilizar/adaptar el handshake temp de mtcute en un probe aislado donde dispositivo genera y conserva la temporary auth key, binder controlado recibe solo metadata mínima y usa permanent auth para construir el binding, y dispositivo ejecuta `auth.bindTempAuthKey` **dispositivo -> Telegram** y después una operación RPC directa. No se tocará runtime productivo hasta que este flujo sea reproducible.
- Después siguen renovación, expiración anticipada, operación larga, **1.9 GB**, Windows/macOS/Web pura, delete cross-bot, cross-vault/shared-bot y escalabilidad/admission control.
