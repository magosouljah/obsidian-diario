...
- [ 🟡 ] Aprobar una persistencia transaccional durable con migrations, constraints, backup/restore y rollback como requisito de producción. **Arquitectura aprobada por RO el 25 de agosto de 2026; implementación técnica parcial integrada en PRs #30–#34; falta cutover productivo y evidencia de infraestructura real.**
...
- [ 🟡 ] Definir cifrado de secretos, migraciones, backup, RPO/RTO propuestos y rollback. **Definido e implementado parcialmente:** envelope encryption AES-256-GCM para secretos recuperables; KEK/master debe quedar fuera de PostgreSQL en KMS/Secret Manager real; objetivo RPO <=15 min y RTO <=2 h; importador idempotente y exporter legacy-compatible existen, pero PITR/WAL/KMS y RPO/RTO productivos siguen sin evidencia.
- [ 🟡 ] Definir reconciliación Telegram/index y garbage journal. **Definido e implementado parcialmente:** INDEX gana; PG stale se repara hacia INDEX; missing referenced object alerta sin reescribir verdad; unreferenced object pasa a orphan candidate tras safety window; cleanup físico es post-commit durable; `MESSAGE_DELETE_FORBIDDEN` nunca revierte INDEX ni resucita contenido. Falta completar el worker runtime durable/retries/leases y adversarial final.
...
- PR #34 fue mergeado como `27ccdcbf41bbce0265730754cc2ff32fd32dc140`. CI post-merge #256 (`32946267360`) terminó PASS completo: PostgreSQL live + recovery, Supply Chain, Web/shared, Windows, macOS arm64, macOS x86_64 y `Required CI`. El probe heredado más reciente #55 (`32946271700`) también terminó PASS sobre el mismo SHA.
- Auditoría read-only posterior al merge confirmó el hueco de cutover: `server.js` solo arranca/migra PostgreSQL cuando `BEATGALER_POSTGRES_ENABLED=true`, pero después carga `server-core.js`; éste aún usa `accounts-data.json` para users/sessions/providers/MFA/entitlements/vault account metadata y `cloud-data.json` para linkedAccounts/uploadedFiles/beatTopics/pendingTopicDeletes/messageRedirects. Por tanto **PostgreSQL todavía no es authority del runtime productivo** y no hay dual-write aprobado.
...
- **Pendiente antes de cerrar 5.2:** cutover explícito/default-off `json|postgres` y repositories runtime PG; migration/cutover real con snapshot immutable+SHA256+dry-run+quarantine+freeze+delta+switch; rollback post-write sin pérdida; worker durable de garbage/reconciliation; adversarial final; proveedor PostgreSQL/KMS real, backup cifrado, WAL/PITR/retención y restore independiente productivo que demuestre RPO <=15 min/RTO <=2 h. **5.2 permanece `[ 🟡 ]`; NO-GO.**

**Dependencias:** contención terminada.  
**Evidencia:** ADR/threat model/migration plan + PRs #29–#34 + CI PostgreSQL real/recovery + auditoría read-only del runtime actual.  
