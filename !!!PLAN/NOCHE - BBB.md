...
- `PREDECESSOR: NIGHT-BBB-105 = BLOCKED_STOP / HARNESS_ONLY_PROVEN_REFRESH_UNSAFE.`
...
- `SERIALIZATION: BBB106 owns #84 evidence/harness only. AAA107 owns F2/13.2. WOZ110 owns #92/integration lane. No product-auth mutation, #92/#89, Review/Trash, provider/deploy or integration mutation.`
...
2. Explicit authority for this successor: build a clean history-preserving successor of #84 from the live baseline, preserving only the authorized #84 evidence/harness delta and the exact intended #74 product-corrective lineage already under test. Do not rewrite integration and do not broaden product scope.
3. Because `HARNESS_ONLY_PROVEN` is established, apply the minimum harness/service boundary correction so WDIO/Tauri plugin IPC is not consumed by the auth HTTP mock/interceptor. Keep `/auth/health` + `/auth/login` stubs and literal product assertions unchanged.
...
7. One bounded PR/evidence lineage only. **NO MERGE CYCLE 111.**
...
- baseline observed: `134a293985c314eb09c238115e3bcb71e79f1810`.
- #84 remained `f53d46f...` on stale base `816f946c...`.
- No mutation/test rerun occurred because the assignment correctly STOPped at unsafe refresh.
- Reusable causal evidence: broad fetch interception is the harness boundary swallowing WDIO/Tauri plugin IPC; product mutation was not proven necessary.
- Still UNVERIFIED: refreshed exact candidate, IPC bypass, session token persistence, AccountGate exit and fresh packaged Windows Auth PASS.
